...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.product_id, p.product_title, p.product_price,p.product_qty, p.product_desc, p.product_image, c.cat_title, c.cat_id, b.brand_id, b.brand_title FROM products p JOIN categories c ON c.cat_id = p.cat_id JOIN brands b ON b.brand_id = p.brand_id");
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Amazon/product_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`cat_id`, `brand_id`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName')");
