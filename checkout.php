...
<section class="section">       
	<div class="container-fluid">
		...
		if(isset($_SESSION["uid"])){
			$sql = "SELECT * FROM user_info WHERE email ='$_SESSION[uid]'";
			...
			<div class="col-25">
				...
				if (isset($_POST["cmd"])) {
					...
					while($i<=$total_count){
						...
						$quantity_ = $_POST['quantity_'.$i];
						$total2=$quantity_*$amount_ ;

						$total=$total2+$total;
						$y = number_format($total2,2);
						$x = number_format($total,2);
						$oo = number_format($quantity_);
						$sql = "SELECT product_id FROM products WHERE product_title='$item_name_'";
						$query = mysqli_query($con,$sql);
						$row=mysqli_fetch_array($query);
						$product_id=$row["product_id"];
						echo "	
						<tr><td><p>$item_number_</p></td><td><p>$item_name_</p></td><td ><p>$oo</p></td><td ><p>$y</p></td></tr>";
						$i++;
					}
				echo"
				</tbody>
				...
				<hr>	
				<h3>total<span class='price' style='color:black'><b>$x</b></span></h3>";
